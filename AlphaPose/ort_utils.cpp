...
//*************************************** ortcv::utils **********************************************//
Ort::Value ortcv::utils::create_tensor(const cv::Mat& mat,
...
cv::Mat ortcv::utils::normalize(const cv::Mat& mat, float mean, float scale)
{
    cv::Mat matf;
...
cv::Mat ortcv::utils::normalize(const cv::Mat& mat, const float* mean, const float* scale)
{
    cv::Mat mat_copy;
...
void ortcv::utils::normalize(const cv::Mat& inmat, cv::Mat& outmat,
    float mean, float scale)
{
    outmat = ortcv::utils::normalize(inmat, mean, scale);
}

void ortcv::utils::normalize_inplace(cv::Mat& mat_inplace, float mean, float scale)
{
    if (mat_inplace.type() != CV_32FC3) mat_inplace.convertTo(mat_inplace, CV_32FC3);
    ortcv::utils::normalize(mat_inplace, mat_inplace, mean, scale);
}

void ortcv::utils::normalize_inplace(cv::Mat& mat_inplace, const float* mean, const float* scale)
{
    if (mat_inplace.type() != CV_32FC3) mat_inplace.convertTo(mat_inplace, CV_32FC3);
